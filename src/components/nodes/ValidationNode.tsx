...
export const ValidationNode = memo(({ id, data, isConnectable, selected }: NodeProps<{ text_content: string }>) => {
  ...
  return (
    <div className="w-[320px] rounded-xl border-2 border-emerald-500 bg-white/90 p-3 shadow-sm backdrop-blur-sm">
      <Handle type="target" position={Position.Left} isConnectable={isConnectable} />
      <div className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-emerald-600">Validation</div>
      <textarea
        ref={textareaRef}
        className="nodrag w-full resize-none overflow-hidden rounded-md border border-slate-200 bg-white p-2 text-sm text-slate-800 shadow-inner placeholder:text-slate-400 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
        value={textContent}
